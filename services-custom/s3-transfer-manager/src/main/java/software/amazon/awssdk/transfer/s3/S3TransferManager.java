...
public interface S3TransferManager extends SdkAutoCloseable {
    ...
    /**
     * Downloads an object from S3 using a presigned URL to a local file.
     * <p>
     * This method provides the same functionality as {@link #downloadFile(DownloadFileRequest)} but uses a presigned URL
     * instead of requiring AWS credentials. This is useful for scenarios where you want to download objects without
     * exposing AWS credentials or when working with presigned URLs generated by other systems.
     * <p>
     * The SDK will create a new file if the provided one doesn't exist. If the file already exists, the SDK will replace it.
     * In the event of an error, the SDK will <b>NOT</b> attempt to delete the file, leaving it as-is.
     * <p>
     * Users can monitor the progress of the transfer by attaching a {@link TransferListener}. The provided
     * {@link LoggingTransferListener} logs a basic progress bar; users can also implement their own listeners.
     * <p>
     ...
     *         S3TransferManager transferManager = S3TransferManager.create();
     *
     *         PresignedDownloadFileRequest downloadRequest =
     ...
     * }
     *
     * @param presignedDownloadFileRequest the presigned URL download request
     ...
     */
    default FileDownload downloadFileFromPresignedUrl(Consumer<PresignedDownloadFileRequest.Builder> request) {
        return downloadFile(PresignedDownloadFileRequest.builder().applyMutation(request).build());
    }

    /**
     * Downloads an object from S3 using a presigned URL through the given {@link AsyncResponseTransformer}.
     * <p>
     * This method provides the same functionality as {@link #download(DownloadRequest)} but uses a presigned URL
     * instead of requiring AWS credentials. This is useful for scenarios where you want to download objects without
     * exposing AWS credentials or when working with presigned URLs generated by other systems.
     * <p>
     * Users can monitor the progress of the transfer by attaching a {@link TransferListener}. The provided
     ...
     * See the static factory methods available in {@link AsyncResponseTransformer} for other use cases.
     *
     * @param presignedDownloadRequest the presigned URL download request
     * @param <ResultT>       The type of data the {@link AsyncResponseTransformer} produces
     * @return A {@link Download} that can be used to track the ongoing transfer
     * @see #downloadFile(PresignedDownloadFileRequest)
     */
