    ...
    public static void main(String[] args) throws MalformedURLException {
        ...
        S3AsyncClient multipartClient = S3AsyncClient.builder()
            .multipartEnabled(true)
                                                     ...
                                                     .overrideConfiguration(f ->f.addExecutionInterceptor(new HeaderLoggingInterceptor())).build();
...
//        System.out.println("byteArray " + byteArray.length);
        ...
        AsyncPresignedUrlExtension asyncPresignedUrlExtension = multipartClient.presignedUrlExtension();
...
//                                                  AsyncResponseTransformer.toBytes()).join().asByteArray();
        ...
        System.out.println("This is for normal range test");
        S3AsyncClient client = S3AsyncClient.builder()
                                                     .overrideConfiguration(f ->f.addExecutionInterceptor(new HeaderLoggingInterceptor())).build();

        AsyncPresignedUrlExtension client2 = client.presignedUrlExtension();
        byte[] byteArray2 = client2.getObject(r -> r.presignedUrl(presignedUrl).range("bytes=20971520-31457279"),
                                                                AsyncResponseTransformer.toBytes()).join().asByteArray();


        System.out.println("byteArray  " +byteArray2.length);
